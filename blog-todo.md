# Blog To-Do List: "How to Use" Marketing Guides 2026

## Instructions for Claude
When resuming this task:
1. **Read existing blog template** from any "how-to-use-*.html" file in `/blog/` folder
2. **Follow the same HTML structure** including: SEO meta tags, FAQ schema, table of contents, tables, key takeaways, CTA section
3. **After creating each blog**, add it to:
   - `/blog/index.html` (add blog card at the end before `</div>` closing blog-grid)
   - `/sitemap.xml` (add URL entry before `<!-- Terms and Conditions -->` comment)
4. **Push to GitHub** after each blog with descriptive commit message
5. **Mark as [DONE]** in this file after completing each blog

---

## Cutting-Edge AI Tools (Low Competition, High Interest)

- [DONE] How to Use Perplexity for Marketing Research 2026
  - File: `how-to-use-perplexity-marketing-research-2026.html`
  - Focus: AI search, competitive research, content research, real-time insights

- [DONE] How to Use Sora for Video Ad Creation 2026
  - File: `how-to-use-sora-video-ads-2026.html`
  - Focus: AI video generation, ad creatives, social video content

- [DONE] How to Use Runway ML for Marketing Videos 2026
  - File: `how-to-use-runway-ml-marketing-2026.html`
  - Focus: Video editing, Gen-3 Alpha, motion brush, video effects

- [DONE] How to Use Eleven Labs for Audio Marketing 2026
  - File: `how-to-use-eleven-labs-marketing-2026.html`
  - Focus: AI voiceovers, podcast production, audio ads, dubbing

- [DONE] How to Use Cursor AI for Marketing Automation 2026
  - File: `how-to-use-cursor-ai-marketing-2026.html`
  - Focus: AI coding, building marketing tools, automation scripts

- [DONE] How to Use Devin AI for Marketing Development 2026
  - File: `how-to-use-devin-ai-marketing-2026.html`
  - Focus: AI software engineer, building marketing applications

- [DONE] How to Use Operator (OpenAI) for Marketing Tasks 2026
  - File: `how-to-use-openai-operator-marketing-2026.html`
  - Focus: Browser automation, task execution, web-based marketing tasks

---

## High-Volume Marketing Tools (Competitive but Essential)

- [DONE] How to Use Canva AI for Marketing Design 2026
  - File: `how-to-use-canva-ai-marketing-2026.html`
  - Focus: Magic Studio, AI design, templates, brand kits

- [DONE] How to Use Figma for Marketing Teams 2026
  - File: `how-to-use-figma-marketing-2026.html`
  - Focus: Design collaboration, prototyping, marketing assets

- [DONE] How to Use Webflow for Marketing Websites 2026
  - File: `how-to-use-webflow-marketing-2026.html`
  - Focus: No-code websites, landing pages, CMS, SEO

- [DONE] How to Use Framer for Landing Pages 2026
  - File: `how-to-use-framer-landing-pages-2026.html`
  - Focus: AI website builder, animations, conversion optimization

- [DONE] How to Use Loom for Sales & Marketing 2026
  - File: `how-to-use-loom-sales-marketing-2026.html`
  - Focus: Video messaging, sales outreach, async communication

---

## Data & Analytics (High Intent Searches)

- [DONE] How to Use Mixpanel for Marketing Analytics 2026
  - File: `how-to-use-mixpanel-marketing-2026.html`
  - Focus: Product analytics, funnel analysis, user behavior

- [DONE] How to Use Amplitude for Product Marketing 2026
  - File: `how-to-use-amplitude-marketing-2026.html`
  - Focus: Product analytics, cohort analysis, experimentation

- [DONE] How to Use Segment for Marketing Data 2026
  - File: `how-to-use-segment-marketing-2026.html`
  - Focus: CDP, data integration, audience building

- [DONE] How to Use Supermetrics for Marketing Reporting 2026
  - File: `how-to-use-supermetrics-reporting-2026.html`
  - Focus: Data connectors, automated reporting, dashboards

---

## Sales & Outreach (B2B Focus)

- [DONE] How to Use Apollo.io for Lead Generation 2026
  - File: `how-to-use-apollo-lead-generation-2026.html`
  - Focus: B2B database, prospecting, email sequences, enrichment

- [DONE] How to Use Instantly for Cold Email 2026
  - File: `how-to-use-instantly-cold-email-2026.html`
  - Focus: Email warmup, deliverability, sequences, lead management

- [DONE] How to Use Lemlist for Email Outreach 2026
  - File: `how-to-use-lemlist-outreach-2026.html`
  - Focus: Personalization, multichannel, LinkedIn + email

- [ ] How to Use Smartlead for Cold Outreach 2026
  - File: `how-to-use-smartlead-outreach-2026.html`
  - Focus: Unlimited mailboxes, AI warmup, unified inbox

---

## Emerging AI Agents (Newest, Least Competition)

- [ ] How to Use Lovable for Marketing Apps 2026
  - File: `how-to-use-lovable-marketing-apps-2026.html`
  - Focus: AI app builder, internal tools, marketing dashboards

- [ ] How to Use Bolt.new for Marketing Websites 2026
  - File: `how-to-use-bolt-new-marketing-2026.html`
  - Focus: AI website builder, full-stack apps, rapid prototyping

- [ ] How to Use v0 by Vercel for Marketing UI 2026
  - File: `how-to-use-v0-vercel-marketing-2026.html`
  - Focus: AI UI generation, React components, landing pages

- [ ] How to Use Replit Agent for Marketing Tools 2026
  - File: `how-to-use-replit-agent-marketing-2026.html`
  - Focus: AI coding, building tools, deployment, collaboration

---

## Progress Tracker

- Total Blogs: 24
- Completed: 19
- Remaining: 5

Last Updated: 2026-01-26
Last Blog Created: how-to-use-lemlist-outreach-2026.html
